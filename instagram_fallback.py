...
class InstagramPublicFallback:
    ...
    PHOTO_SERVICES = (
        # Direct-link/one-paste services first because they explicitly support public
        # Instagram posts and return downloadable JPG/MP4 assets.
        ("InstaLoadr-direct", "https://www.instaloadr.com", ("/{url}" , "/?url={url}")),
        ("Tikt-direct", "https://tikt.com", ("/{url}", "/instagram/?url={url}")),
    ...
    DOWNLOAD_WORDS = ("download", "media", "image", "photo", "video", "story", "cdn", "scontent", "fbcdn")
    ...
    @staticmethod
    def _is_instagram_media_page(url):
    ...
    def _looks_like_media_url(cls, value):
        ...
        if any(path.endswith(ext) for ext in cls.MEDIA_EXTENSIONS):
            return True
        # Only accept extension-less URLs when they strongly resemble a media/CDN URL.
    ...
    def _visit_variants(self, service_name, base, patterns, username, instagram_url):
        urls = []
        for pattern in patterns:
            urls.append(base + pattern.format(username=quote_plus(username or ""), url=quote_plus(instagram_url)))
        return urls
    ...
    def fetch_story(self, instagram_url, output_dir):
        ...
        for name, base, patterns in self.STORY_SERVICES:
            for page_url in self._visit_variants(name, base, patterns, username, instagram_url):
                try:
    ...
    def fetch_post(self, instagram_url, output_dir):
        """Try dedicated public post/photo downloaders for /p/, /reel/, /reels/, /tv/."""
        errors = []

        # Direct Instagram page first for OG/Twitter/JSON metadata.
        candidates = [instagram_url, instagram_url.split("?", 1)[0]]
        ...
        for name, base, patterns in self.PHOTO_SERVICES:
            for page_url in self._visit_variants(name, base, patterns, self.username_from_url(instagram_url) or "", instagram_url):
                try:
                    page = self.session.get(page_url, timeout=self.timeout, allow_redirects=True)
                    if page.status_code >= 400:
                        continue
